...
bool SystemScreenSaver::allowSleep() {
    //return false;
    return ((mVideoScreensaver == NULL) && (mImageScreensaver == NULL) && (mVideoDetail == NULL));
}
...
void SystemScreenSaver::startScreenSaver() {
    ...
    if ((!mVideoScreensaver || !mVideoDetail) && (screensaver_behavior == "random video")) {
        ...
        if (!path.empty() && Utils::FileSystem::exists(path)) {
            if (Settings::getInstance()->getString("StretchVideoOnScreenSaver") == "detail") {
                gamesHistorySize = std::round(mVideoCount * 0.7);
                bool init = false;
                if (gamesHistoryIndex >= gamesHistorySize) {
                    gamesHistoryIndex = 0;
                    last = 0;
                    init = true;
                }
                if (init || (gamesHistoryIndex >= last + 1) || gamesHistoryIndex >= gamesHistory.size()) {
                    if (gamesHistory.size() < gamesHistorySize) {
                        gamesHistory.insert(gamesHistory.begin() + gamesHistoryIndex, mCurrentGame);
                    } else {
                        gamesHistory.at(gamesHistoryIndex) = mCurrentGame;
                    }

                    last = gamesHistoryIndex;

                }

                mVideoDetail = new VideoDetail(mWindow, gamesHistory[gamesHistoryIndex]);
                gamesHistoryIndex++;
...
void SystemScreenSaver::renderScreenSaver() {
    ...
    if ((mVideoScreensaver || mVideoDetail) && screensaver_behavior == "random video") {
        ...
        if ((int) mState >= STATE_FADE_IN_VIDEO) {
            ...
            Settings::getInstance()->getString("StretchVideoOnScreenSaver") == "detail" ?
            mVideoDetail->render(transform) : mVideoScreensaver->render(transform);
        }
...
void SystemScreenSaver::pickRandomVideo(std::string &path) {
    ...
    if (Settings::getInstance()->getString("StretchVideoOnScreenSaver") == "detail") {
        if (mVideoCount > 0) {
            bool alreadyInHistory;
            do {
                int video = (int) (((float) rand() / float(RAND_MAX)) * (float) mVideoCount);
                pickGameListNode(video, "video", path);
                alreadyInHistory =
                        std::find(gamesHistory.begin(), gamesHistory.end(), mCurrentGame) != gamesHistory.end();
            } while (alreadyInHistory);
        }
    }
    else {
        if (mVideoCount > 0) {
...
void SystemScreenSaver::pickRandomCustomImage(std::string &path) {
    ...
    if ((imageDir != "") && (Utils::FileSystem::exists(imageDir))) {
        ...
        Utils::FileSystem::stringList dirContent = Utils::FileSystem::getDirContent(imageDir,
